...
class NotificationService {
  public async createNotification(data: {
    ...
    try {
      ...
      logger.info(
        `Notification created for user ${data.userId}: ${data.message}`
      );
  ...
  public async notifyProfileLike(
    likedUserId: string,
    likerUserId: string
  ): Promise<void> {
    const liker = await User.findById(likerUserId);
    const liked = await User.findById(likedUserId);
    if (!liker || !liked) return;
  ...
  public async notifyProfileComment(
    profileUserId: string,
    commenterUserId: string
  ): Promise<void> {
    const commenter = await User.findById(commenterUserId).select(
      "firstName lastName surname"
    );
    const commented = await User.findById(profileUserId).select(
      "firstName lastName surname"
    );
    if (!commenter || !commented) return;
  ...
  public async notifyPostLike(
    postOwnerId: string,
    likerUserId: string,
    postId: string
  ): Promise<void> {
    const liker = await User.findById(likerUserId).select(
      "firstName lastName surname"
    );
    if (!liker) return;
  ...
  public async notifyUserTag(
    taggedUserId: string,
    taggerUserId: string
  ): Promise<void> {
    const tagger = await User.findById(taggerUserId).select(
      "firstName lastName surname"
    );
    if (!tagger) return;
  ...
  public async notifyPostShare(
    postOwnerId: string,
    sharerUserId: string,
    postId: string
  ): Promise<void> {
    const sharer = await User.findById(sharerUserId).select(
      "firstName lastName surname"
    );
    if (!sharer) return;
