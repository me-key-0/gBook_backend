...
const seedData: SeedData = {
  ...
  questions: [
    ...
    {
      question:
        "What was the most challenging moment in your university journey?",
      type: "post",
      category: "challenges",
      isRequired: false,
      order: 1,
...
class DatabaseSeeder {
  ...
  async seedDepartments(): Promise<void> {
    ...
    const departmentsWithCollege = seedData.departments.map(
      (department, index) => ({
        ...department,
  ...
  async seedUsers(): Promise<void> {
    ...
    // Create sample guests - FIXED: Added proper surnames
    const guests = [];
    const guestSurnames = [
      "Guest",
      "Visitor",
    ...
    for (let i = 0; i < 10; i++) {
      const firstName = firstNames[i % firstNames.length];
      const lastName = lastNames[i % lastNames.length];
      const surname = guestSurnames[i % guestSurnames.length]; // Use proper surname instead of empty string

      guests.push({
        firstName,
        lastName,
        surname, // This now has a valid value (at least 2 characters)
    ...
    const allUsers = [
      adminUser,
      ...(await User.insertMany([...graduates, ...guests])),
    ];
    this.userIds = allUsers.map((user) => user._id.toString());

    logger.info(
      `Seeded ${allUsers.length} users (1 admin, ${graduates.length} graduates, ${guests.length} guests)`
    );
  }
  ...
  async seedPosts(): Promise<void> {
    ...
    const lastWordQuestions = this.questionIds.filter(
      (_, index) => seedData.questions[index]?.type === "lastword"
    );
    ...
    const postQuestions = this.questionIds.filter(
      (_, index) => seedData.questions[index]?.type === "post"
    );

    for (let i = 0; i < 30; i++) {
      const userId = graduateUsers[i % graduateUsers.length];
      const questionId =
        postQuestions[i % postQuestions.length] ||
