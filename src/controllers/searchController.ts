...
class SearchController {
  ...
  public getSearchFilters = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const graduationYears = await User.aggregate([
        { $match: { isActive: true } },
        {
          $group: {
            _id: null,
            min: { $min: "$graduationYear" },
            max: { $max: "$graduationYear" },
          },
        },
      ]);

      const yearRange = graduationYears[0] || {
  ...
  public getPopularSearches = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const { limit = 10 } = req.query;

      const [popularCampuses, popularColleges, popularDepartments] =
        await Promise.all([
  ...
  public getQuickSearch = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      ResponseHandler.success(
        res,
        { users, posts },
        "Quick search results retrieved successfully"
      );
    }
  );

  public getRecentSearches = asyncHandler(
    async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      const currentUserId = req.userId!;
  ...
  public saveSearch = asyncHandler(
    async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      const currentUserId = req.userId!;
      ...
      logger.info(
        `Search saved for user ${currentUserId}: query="${query}", filters=${JSON.stringify(
          filters
        )}`
      );
